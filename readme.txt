...
○注意事項
　・正規表現のオプションは検索文字列のみ正規表現が使用可能となります。
　　置換文字列で正規表現を記載した場合はただの文字列として扱われます。
...
○アップデート予定機能(リリース時期は未定)
既存機能のアップデート
・ブック内検索にセルの文字列を検索できる機能(現在着手中)
・置換で正規表現でのキャプチャ置換を可能に
・検索で書式での検索を可能に
新規追加機能(検討中)
・Winmergeを使用し差分のレポートを出力し、レポートをエクセルに張り付ける機能
　貼り付け時にフォーマットが指定可能(Winmergeのレポート以外の部分)
